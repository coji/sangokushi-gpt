...
export const createQdrant = (server: string, port = 6333) => {
   ...
   * コレクションを作成する
   */
  const createCollection = async ({ collection, vectors }: QdrantCollectionCreateParams) => {
    const ret = await fetch(`http://${server}:${port}/collections/${collection}`, {
      method: 'PUT',
  ...
  const addPoints = async ({ collection, points }: QdrantAddPointsParams) => {
    const ret = await fetch(`http://${server}:${port}/collections/${collection}/points?wait=true`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (!ret.ok) {
      throw new Error(
        ...
        `Failed to add points: ${ret.statusText} ${JSON.stringify(await ret.json())}`,
      )
    }
  ...
  const search = async <T>({
    ...
    const ret = await fetch(`http://${server}:${port}/collections/${collection}/points/search`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        ...params,
